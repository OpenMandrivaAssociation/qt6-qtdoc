...
%else
Source:		https://download.qt.io/%{?beta:development}%{!?beta:official}_releases/qt/%(echo %{version}|cut -d. -f1-2)/%{version}%{?beta:-%{beta}}/submodules/qtdoc-everywhere-src-%{version}%{?beta:-%{beta}}.tar.xz
%endif
...
BuildRequires:	cmake(Qt%{major}Quick3DXr)
BuildRequires:	cmake(Qt%{major}Quick3DPhysics)
...
BuildRequires:	cmake(Qt%{major}Graphs)
...
BuildRequires:	cmake(Qt%{major}Sensors)
BuildRequires:	cmake(Qt%{major}Multimedia)
BuildRequires:	cmake(Qt%{major}Charts)
BuildRequires:	cmake(Qt%{major}Pdf)
BuildRequires:	cmake(Qt%{major}PdfWidgets)
BuildRequires:	cmake(Qt%{major}ExamplesAssetDownloaderPrivate)
